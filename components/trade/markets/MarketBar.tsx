...
export function MarketBar({ market}: { market :  string }) {
  const { marketData, priceChangeClassName } = UseMarketWebsockets(market)
  ...
  return (
    <Card className="bg-card text-card-foreground p-4 overflow-hidden border-none rounded-none">
      <CardContent className="p-2">
        <div className="flex items-center space-x-4 overflow-x-auto whitespace-nowrap gap-2">
          <div className="flex-shrink-0 flex items-center space-x-2">
            ...
            <span className={`text-lg font-bold ${priceChangeClassName}`}>
              {parseFloat(marketData.c).toFixed(2)}
            </span>
          </div>
          <div className={`flex-shrink-0 flex items-center space-x-1 ${priceChangeClassName}`}>
          ...
          <div className="flex-shrink-0 flex items-center space-x-4 text-xs">
            ...
            <div className="flex flex-col">
              <span className="text-muted-foreground">24h Vol({market.slice(0, 3)})</span>
              <span>{parseFloat(marketData.v).toFixed(2)}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-muted-foreground">24h Vol({market.slice(3)})</span>
